...
Most source files (`src/`, `package.json`, `tsconfig.json`, etc.) are generated from the upstream monorepo. PRs that modify these files can't be merged directly because the next sync would overwrite them.
